# 命运:冠位指定
### 前面
我从鬼岛入坑,700W下载弃坑,为了刷技能,写了个自动刷材料的脚本,现在用不上了.因为是自用,没有做适配,用的是触动精灵,分辨率只支持1920*1080,需要root.不支持多血条的识别,有些活动中关底BOSS血条特别长,也是不支持的.脚本主要用在借好友的打手然后单刷,或者是不能稳定3回合之后的补刀.
### 刷友情点
